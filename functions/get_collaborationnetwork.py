...
def get_collaboration_network(
    ...
    """
    Generate a collaboration network (authors, universities, countries) similar to the R socialStructure function.
    ...
    df : DataFrame
        Bibliographic data.
    field : str
        Collaboration field ('COL_AU', 'COL_UN', 'COL_CO').
    sep : str
        Separator for references.
    network_layout : str
        Network layout type.
    clustering_algorithm : str
        Clustering algorithm.
    repulsion : float
        Repulsion force for community layout.
    shape : str
        Node shape.
    opacity : float
        Node opacity (alpha).
    ...
    # Build the collaboration network based on field
    if field == "COL_AU":
        NetRefs = biblionetwork(M, analysis="collaboration", network="authors", n=nodes)
        Title = "Author Collaboration network"
    elif field == "COL_UN":
        if "AU_UN" not in m.columns:
            M = metaTagExtraction(M, Field="AU_UN")
        NetRefs = biblionetwork(M, analysis="collaboration", network="universities", n=nodes)
        Title = "Edu Collaboration network"
    elif field == "COL_CO":
        if "AU_CO" not in m.columns:
            M = metaTagExtraction(M, Field="AU_CO")
        NetRefs = biblionetwork(M, analysis="collaboration", network="countries", n=nodes)
    ...
    unique_clusters = set(netplot['cluster_obj'].membership)
    cluster_colors = {}
    ...
    node_labels = [v["name"] if "name" in v.attributes() else f"Node {v.index}" for v in netplot['graph'].vs]
    node_sizes = []
    nodes_list = []
    for idx, vertex in enumerate(netplot['graph'].vs):
        cluster_id = netplot['cluster_obj'].membership[vertex.index]
        node_color = cluster_colors[cluster_id]
        min_deg, max_deg = min(netplot['graph'].degree()), max(netplot['graph'].degree())
        ...
        nodes_list.append({
            ...
            'font': {
                ...
                'vadjust': -0.7 * font_size if shape.lower() in ['dot', 'square'] else 0
            },
            'shadow': shadow,
            'shape': shape,
            'x': layout[idx][0] * 1000,
    ...
    unique_nodes = {node['id']: node for node in nodes_list}.values()
    ...
    added_edges = set()
    edge_weights = [e.attributes().get('weight', 1) for e in netplot['graph'].es]
    max_weight = max(edge_weights) if edge_weights else 1

    for edge in netplot['graph'].es:
        source, target = edge.tuple
        cluster_source = netplot['cluster_obj'].membership[source]
        cluster_target = netplot['cluster_obj'].membership[target]
        ...
        if edge_tuple not in added_edges:
            net.add_edge(
                ...
                smooth={'type': 'horizontal'} if curved else False,
                dashes=False
            )
    ...
    ##################################### Density Plot #####################################
    nodes_df_orig = pd.DataFrame(nodes_list)
    ...
    cluster_data = pd.DataFrame({
        'Node': [v['name'] if 'name' in v.attributes() else f'Node {v.index}' for v in netplot['graph'].vs],
    ...
    netplot['cluster_res'] = cluster_data

    # Degree plot
    node_degrees = pd.DataFrame({
        'node': [v['name'] if 'name' in v.attributes() else f'Node {v.index}' for v in netplot['graph'].vs],
        'degree': netplot['graph'].degree()
    })
    ...
    return html_path.split(os.sep)[-1], fig_density, netplot['cluster_res'], degree_plot
